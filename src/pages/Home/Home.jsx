...
import { getFilmsByName } from "../../redux/store/actions/film.action";
...
function Home() {
  ...
  useEffect(() => {
    if (debouncedSearchTerm) {
      dispatch(getFilmsByName(debouncedSearchTerm));

      film.Search === undefined
        ? setMoviesList([])
        : setMoviesList(film.Search);

      setHasError(false);
      setTotalResults(film.totalResults);
      setNextPage(2);
    }
  }, [debouncedSearchTerm]);
